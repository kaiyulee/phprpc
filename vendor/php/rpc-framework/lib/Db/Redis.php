...
class Redis {
    ...
    /**
     * 查看redis连接是否断开
     * @return bool $return bool true:连接未断开 false:连接已断开
     */
    ...
    {
        return $this->_REDIS->setOption($key, $value);
    }
    ...
    {
        return $this->_REDIS->getOption($key);
    }
     ...
     * @param float|int $time float 过期时间(S)
     * @param $old int 1:返回旧的value 默认0
     * @return bool|int|null|string $return bool true:成功 flase:失败
     */
     ...
     * @return array|bool|null|string $return mixed 如果key存在则返回key值 如果不存在返回false
     */
     ...
     * @return null|void $return longint 删除成功的key的个数
     */
     ...
     * @return bool|int|null
     * @internal param int $deriction 0:数据入队列头(左) 1:数据入队列尾(右) 默认为0
     */
    ...
    {
        return $this->_REDIS->hGet($keyName,$fieldName);
    }
    /**
     * 获取hash表中元素个数
     * @param $hash string 哈希表名
     ...
     * 提交完成事务
     * @return
     * @internal param bool $return 事务执行成功 提交操作
     */
     ...
     * 回滚事务
     * @return
     * @internal param bool $return 事务执行失败 回滚操作
     */
